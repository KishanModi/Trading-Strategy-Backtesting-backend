...
def get_test_data(symbol,syear,eyear):
    #stock_name = input("Enter Stock Name :---  ")
    stock_name =symbol
    start_date = datetime.date(syear, 1, 1)
    end_date = datetime.date(eyear, 1, 1)
...
def execute(model,symbol,syear,eyear):
    # TODO get prediction data
    test = get_test_data(symbol,syear,eyear)
...
# Returns    :- DataFrame containing output predicted by ML model
def calls(symbol,syear,eyear):
    model = getModel()
    return execute(model,symbol,syear,eyear)